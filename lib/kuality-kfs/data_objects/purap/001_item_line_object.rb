class ItemLineObject < DataFactory
  ...
  def defaults
    default_item_accounting_lines.merge(get_aft_parameter_values_as_hash(ParameterConstants::DEFAULTS_FOR_ITEMS))
  end
  ...
  def determine_commodity_code_of_type(commodity_type='non-sensitive')
    # Cannot randomly select out of all the options available on the sensitive data dropdown
    # because not all options have data associated with them. AFT will fail when no data provided.
    # Dropdown is zero based. Positions zero, one, and four have data values associated to them.
    # Position zero returns any non-sensitive commodity code represented by this method's return
    # value of an empty string.
    # As of March 2016, positions one and four return commodity codes defined as sensitive.
    # In order for AFTs to be able to randomly use either value, an odd-even randomization
    # was implemented with DEFAULTS_FOR_ITEMS odd_commodity and even_commodity used respectively.
    set_dropdown_to = ''
    if !(commodity_type.eql?('non-sensitive'))
      if (rand(500)).odd?
        set_dropdown_to = (get_aft_parameter_values_as_hash(ParameterConstants::DEFAULTS_FOR_ITEMS))[:odd_commodity_lookup]
      else
        set_dropdown_to = (get_aft_parameter_values_as_hash(ParameterConstants::DEFAULTS_FOR_ITEMS))[:even_commodity_lookup]
      end
    end
    set_dropdown_to
  end
  ...
  def pull_existing_items(i=0, t=:new)
    ...
    on ItemsTab do |b|
      case t
        when :new
          pulled_item = {
              type:                 (b.update_type(i).selected_options.first.text.strip           if b.update_type(i).exists?),
              quantity:             (b.update_quantity(i).value.strip                             if b.update_quantity(i).exists?),
              uom:                  (b.update_uom(i).value.strip                                  if b.update_uom(i).exists?),
              catalog_number:       (b.update_catalog_number(i).value.strip                       if b.update_catalog_number(i).exists?),
