Feature: Asset Payment
  ...
  [KFSQA-969] Submit an allocated asset manual payment where total allocations are not equal to the asset line.
              Test that trying to submit the out of balance payment displays an error.
              Correct the transaction so allocations equal asset line and test payment can be submitted without an error.
